...
def train(train_loader, model, classifier, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, descriptions, labels) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            text_features = model.description_tokenizer.encode(descriptions, 
                                                    convert_to_tensor=True,
                                                    normalize_embeddings=True)
            compose_features = torch.cat([features, text_features], dim=1)
            
        output = classifier(compose_features.detach())
...
def validate(val_loader, model, classifier, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, (images, descriptions, labels) in enumerate(val_loader):
            ...
            output = classifier(compose_features)
            loss = criterion(output, labels)
